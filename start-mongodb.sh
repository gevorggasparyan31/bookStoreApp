...
if docker image inspect "$MONGODB_IMAGE" &>/dev/null; then
...
docker run -d -p 27017:27017 --name my-mongodb "$MONGODB_IMAGE"

# Check if the container is running
if [ "$(docker inspect -f '{{.State.Running}}' my-mongodb 2>/dev/null)" == "true" ]; then
